...
export async function currentWarAnalysis() {
  ...
  return {
    state: war.state, war_key: war.war_key, start_time: war.start_time, end_time: war.end_time,
    members, attacks,
...
export async function memberLeaderboard(metric = 'donations', descending = true) {
  ...
  return players.map(p => ({ name: p.name, tag: p.tag, value: Number(p[field] ?? 0) }))
...
export async function capitalPlayerLeaderboard(seasons = 12) {
  ...
  // Member totals come from the official raid-season member objects.
  ...
  // Attack-level stars/destruction come from the dedicated Capital table.
